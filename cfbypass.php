...
***************WARNING*****************
***************************************
This script is very, very bad.
The use of the get_between function is strictly due to being to lazy to make the regex patterns.
It is so fucking terribly coded, I could make something better but I honestly cba to put in the effort for something that is going to be irrelevant in a few months.
If you're an aspiring PHP coder looking to 'fix' this script, here's a list of things wrong with it:
- get_between. Switch everything to preg_* functions and you'll see a performance increase.
- Not parsing the $domain in 'bypassyourdog' into a protocol, domain and path. Doing this would allow you to finish target url's with a /, whereas you currently can't due to the way the $domain1 variable is set.
- Not checking for a '=' in a string before exploding for it. This creates the ever so dreaded 'Undefined offset 1' error.
- Not checking if the page is actually cloudflare's 5 second UAM challenge before attempting to solve the challenge.
That's all I can think of at 12:20 AM. I might release a better version of this sometime in the coming months if I find myself ridiculously bored.
*/
if($argc != 5) {
